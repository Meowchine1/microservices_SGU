...
func main() {
	...
	db, err := sql.Open("postgres", "user=postgres dbname=orderservice sslmode=disable")
    ...
    qAgreeOrders, err := declareQueue(ch, config.order_queue_name)
	// Объявление очереди для получения уведомлений о создании заказа
	qCreatedOrders, err :=declareQueue(ch, config.order_create_queue_name)
	...
	go func() { 
		for { 
			...
			failOnError(err, "Failed to publish a message to 'order_queue' queue")
			log.Printf(" [x] Order %s created and sent to 'order_queue' queue", orderIDStr)
